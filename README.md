...
// constants/billingTypes.js
export const BILLING_TYPES = {
  PAY_AS_YOU_GO: 'PAY_AS_YOU_GO',
  PREPAID_ONE_TIME: 'PREPAID_ONE_TIME', 
  PREPAID_AUTO_RENEW: 'PREPAID_AUTO_RENEW',
  PREPAID_SEPARATE_ORDERS: 'PREPAID_SEPARATE_ORDERS'
};

export const BILLING_TYPE_LABELS = {
  [BILLING_TYPES.PAY_AS_YOU_GO]: 'Pay As You Go (Auto-renew)',
  [BILLING_TYPES.PREPAID_ONE_TIME]: 'Prepaid One-Time',
  [BILLING_TYPES.PREPAID_AUTO_RENEW]: 'Prepaid Auto-renew',
  [BILLING_TYPES.PREPAID_SEPARATE_ORDERS]: 'Prepaid (Separate Orders)'
};
...
export const FREQUENCY_UNIT_LABELS = {
  [FREQUENCY_UNITS.DAYS]: 'Days',
  [FREQUENCY_UNITS.WEEKS]: 'Weeks',
  [FREQUENCY_UNITS.MONTHS]: 'Months',
  [FREQUENCY_UNITS.YEARS]: 'Years'
};

const recurringDayOptions = [
  { value: "SAME_AS_INITIAL", label: "Same day as initial order" },
  { value: "1", label: "1st of each month" },
  { value: "2", label: "2nd of each month" },
  { value: "3", label: "3rd of each month" },
  // ... continue up to 31
  { value: "31", label: "31st of each month" }
];
...
// Example 2: 50% off first 3 payments, then 10% off
const tieredDiscount = {
  discountEnabled: true,
  discountType: "PERCENT_OFF", 
  discountValue: 50,
  changeDiscountAfterPayments: true,
  changeDiscountAfterCycles: 3,
  newDiscountType: "PERCENT_OFF",
  newDiscountValue: 10
};

// Example 3: $10 off first payment, then $5 off
const amountDiscount = {
  discountEnabled: true,
  discountType: "AMOUNT_OFF",
  discountValue: 10,
  changeDiscountAfterPayments: true, 
  changeDiscountAfterCycles: 1,
  newDiscountType: "AMOUNT_OFF",
  newDiscountValue: 5
};
...
const basicPlan = {
  ...
  cancellationPolicy: "AFTER_INITIAL_ONLY", 
  automaticExpiration: "UNLIMITED"
};

// Example 2: Limited-time subscription  
const limitedPlan = {
  inventoryPolicy: "ON_FULFILLMENT",
  cancellationPolicy: "AFTER_INITIAL_PLUS_3", // Can cancel after 4 total orders
  automaticExpiration: "6_ORDERS_TOTAL" // Ends after 6 orders
};

// Example 3: Trial subscription
const trialPlan = {
  inventoryPolicy: "ON_SALE",
  cancellationPolicy: "AFTER_INITIAL_ONLY",
  automaticExpiration: "1_ORDER_ONLY" // One-time trial
};
...
VALUES
-- ORDER FREQUENCY
  (uuid(), 'abc', 'en', 'ORDER_FREQUENCY', 'ORDER_FREQUENCY_TEXT', 'Order frequency', 'Order frequency', 1),
  (uuid(), 'abc', 'en', 'ORDER_FREQUENCY', 'DELIVERY_INTERVAL_TEXT', 'Delivery Interval', 'Delivery Interval', 2),
  (uuid(), 'abc', 'en', 'ORDER_FREQUENCY', 'EDIT_FREQUENCY_BTN_TEXT', 'Edit Frequency', 'Edit Frequency', 3),
  (uuid(), 'abc', 'en', 'ORDER_FREQUENCY', 'UPDATE_FREQUENCY_BTN_TEXT', 'Update', 'Update', 4),
  (uuid(), 'abc', 'en', 'ORDER_FREQUENCY', 'NO_DELIVERY_FREQUENCY_FOUND_TEXT', 'No frequency available to change.', 'No frequency available to change.', 5),

-- PRODUCT MANAGEMENT
  (uuid(), 'abc', 'en', 'PRODUCT_MANAGEMENT', 'PRODUCT_ACCORDION_TEXT', 'Product in this Subscription', 'Product in this Subscription', 1),
  (uuid(), 'abc', 'en', 'PRODUCT_MANAGEMENT', 'PRODUCTS_LABEL_TEXT', 'Products', 'Products', 2),
  (uuid(), 'abc', 'en', 'PRODUCT_MANAGEMENT', 'SELECT_PRODUCT_LABEL_TEXT', 'Select Product', 'Select Product', 3),

-- SKIP & QUEUE
  (uuid(), 'abc', 'en', 'SKIP_QUEUE', 'SKIP_BADGE_TEXT', 'Skipped', 'Skipped', 1),
  (uuid(), 'abc', 'en', 'SKIP_QUEUE', 'QUEUE_BADGE_TEXT', 'Queued', 'Queued', 2),
  (uuid(), 'abc', 'en', 'SKIP_QUEUE', 'SKIP_ORDER_BUTTON_TEXT', 'Skip Order', 'Skip Order', 3),
  (uuid(), 'abc', 'en', 'SKIP_QUEUE', 'UPCOMING_SKIP_ALERT_TEXT', 'Are you sure you want to skip the next order? Next order date will be changed to {{next_order_date}}.', 'Are you sure you want to skip the next order? Next order date will be changed to {{next_order_date}}.', 4),
  (uuid(), 'abc', 'en', 'SKIP_QUEUE', 'CONFIRM_SKIP_ORDER_TEXT', 'Confirm Skip Order', 'Confirm Skip Order', 5),
  (uuid(), 'abc', 'en', 'SKIP_QUEUE', 'SKIP_FOR_UPCOMING_ORDER_TEXT', 'Skipped for upcoming order', 'Skipped for upcoming order', 6),
  (uuid(), 'abc', 'en', 'SKIP_QUEUE', 'UNSKIP_ORDER_BUTTON_TEXT', 'Unskip Order', 'Unskip Order', 7),
  (uuid(), 'abc', 'en', 'SKIP_QUEUE', 'UNSKIP_ORDER_UPDATE_BUTTON_TEXT', 'Unskip', 'Unskip', 8),
  (uuid(), 'abc', 'en', 'SKIP_QUEUE', 'UNSKIP_ORDER_MODAL_TITLE_TEXT', 'Unskip Order', 'Unskip Order', 9),
  (uuid(), 'abc', 'en', 'SKIP_QUEUE', 'UNSKIP_ORDER_MESSAGE_TEXT', 'Are you sure you want to unskip this order?', 'Are you sure you want to unskip this order?', 10),
  (uuid(), 'abc', 'en', 'SKIP_QUEUE', 'SKIP_FULFILLMENT_BUTTON_TEXT', 'Skip Fulfillment', 'Skip Fulfillment', 11),
  (uuid(), 'abc', 'en', 'SKIP_QUEUE', 'CONFIRM_SKIP_FULFILLMENT_BUTTON_TEXT', 'Are you sure you want to skip this order?', 'Are you sure you want to skip this order?', 12),

-- DELETE CONFIRMATION
  (uuid(), 'abc', 'en', 'DELETE_CONFIRMATION', 'DELETE_CONFIRMATION_MESSAGE_TEXT', 'Are you sure?', 'Are you sure?', 1),
  (uuid(), 'abc', 'en', 'DELETE_CONFIRMATION', 'DELETE_MESSAGE_TEXT', 'You are about to delete this product.', 'You are about to delete this product.', 2),
  (uuid(), 'abc', 'en', 'DELETE_CONFIRMATION', 'YES_BUTTON_TEXT', 'Yes', 'Yes', 3),
  (uuid(), 'abc', 'en', 'DELETE_CONFIRMATION', 'NO_BUTTON_TEXT', 'No', 'No', 4),
  (uuid(), 'abc', 'en', 'DELETE_CONFIRMATION', 'DELETE_BUTTON_TEXT', 'Delete', 'Delete', 5),

-- PAUSE & RESUME
  (uuid(), 'abc', 'en', 'PAUSE_RESUME', 'PAUSE_SUBSCRIPTION_TEXT', 'Pause Subscription', 'Pause Subscription', 1),
  (uuid(), 'abc', 'en', 'PAUSE_RESUME', 'RESUME_SUBSCRIPTION_TEXT', 'Resume Subscription', 'Resume Subscription', 2),
  (uuid(), 'abc', 'en', 'PAUSE_RESUME', 'PAUSE_BADGE_TEXT', 'PAUSED', 'PAUSED', 3),
  (uuid(), 'abc', 'en', 'PAUSE_RESUME', 'CONFIRM_RESUME_TEXT', 'Are you sure you want to resume the subscription?', 'Are you sure you want to resume the subscription?', 4),
  (uuid(), 'abc', 'en', 'PAUSE_RESUME', 'CONFIRM_PAUSE_TEXT', 'Are you sure you want to pause the subscription?', 'Are you sure you want to pause the subscription?', 5),
  (uuid(), 'abc', 'en', 'PAUSE_RESUME', 'ARCHIVED_LABEL_TEXT', 'Archived', 'Archived', 6),
  (uuid(), 'abc', 'en', 'PAUSE_RESUME', 'CANCELED_LABEL_TEXT', 'Canceled', 'Canceled', 7),
  (uuid(), 'abc', 'en', 'PAUSE_RESUME', 'PAUSE_ORDER_CYCLE_TEXT', 'How many orders/cycles you want subscription to pause for?', 'How many orders/cycles you want subscription to pause for?', 8),
  (uuid(), 'abc', 'en', 'PAUSE_RESUME', 'PAUSE_FEEDBACK_TEXT', 'Pause Feedback', 'Pause Feedback', 9),
  (uuid(), 'abc', 'en', 'PAUSE_RESUME', 'PAUSE_TILL_DATE_TEXT', 'Pause Till Date', 'Pause Till Date', 10),
  (uuid(), 'abc', 'en', 'PAUSE_RESUME', 'SELECTED_PAUSE_REASON_TEXT', 'Selected Pause Reason', 'Selected Pause Reason', 11),

-- CANCELLATION
  (uuid(), 'abc', 'en', 'CANCELLATION', 'CANCEL_PREPAID_CONFIRM_TEXT', 'Are you sure you want to cancel your prepaid subscription?', 'Are you sure you want to cancel your prepaid subscription?', 1),
  (uuid(), 'abc', 'en', 'CANCELLATION', 'CANCEL_PAYG_CONFIRM_TEXT', 'Are you sure you want to cancel your subscription?', 'Are you sure you want to cancel your subscription?', 2),
  (uuid(), 'abc', 'en', 'CANCELLATION', 'CANCEL_PREPAID_BUTTON_TEXT', 'Cancel Prepaid Subscription', 'Cancel Prepaid Subscription', 3),
  (uuid(), 'abc', 'en', 'CANCELLATION', 'CANCEL_PAYG_BUTTON_TEXT', 'Cancel Subscription', 'Cancel Subscription', 4),
  (uuid(), 'abc', 'en', 'CANCELLATION', 'CANCEL_ACCORDION_TITLE', 'Cancel Subscription', 'Cancel Subscription', 5),
  (uuid(), 'abc', 'en', 'CANCELLATION', 'SELECT_CANCELLATION_REASON_LABEL', 'Select Cancellation Reason', 'Select Cancellation Reason', 6),
  (uuid(), 'abc', 'en', 'CANCELLATION', 'SELECT_CANCELLATION_REASON_PLACEHOLDER', 'Select Cancellation Reason', 'Select Cancellation Reason', 7),
  (uuid(), 'abc', 'en', 'CANCELLATION', 'CANCELLATION_FEEDBACK_LABEL', 'Cancellation Reason', 'Cancellation Reason', 8),
  (uuid(), 'abc', 'en', 'CANCELLATION', 'CANCELLATION_DATE_TITLE_TEXT', 'Cancellation Date', 'Cancellation Date', 9),
  (uuid(), 'abc', 'en', 'CANCELLATION', 'CANCELLATION_OPTIONAL_HINT_TEXT', '(optional)', '(optional)', 10),
  (uuid(), 'abc', 'en', 'CANCELLATION', 'CANCELLATION_REASON_REQUIRED_MSG', 'Please select a reason for cancellation.', 'Please select a reason for cancellation.', 11),
  (uuid(), 'abc', 'en', 'CANCELLATION', 'CANCELLATION_DISCOUNT_ALREADY_USED_MSG', 'Discount for cancellation already used.', 'Discount for cancellation already used.', 12),

-- FREEZE
  (uuid(), 'abc', 'en', 'FREEZE', 'FREEZE_UPDATE_MESSAGE', 'Subscription cannot be modified until {{minCycles}} orders have completed.', 'Subscription cannot be modified until {{minCycles}} orders have completed.', 1),
  (uuid(), 'abc', 'en', 'FREEZE', 'CONTRACT_FROZEN_MESSAGE', 'Your subscription contract is frozen by your shop owner.', 'Your subscription contract is frozen by your shop owner.', 2),
  (uuid(), 'abc', 'en', 'FREEZE', 'PREVENT_BEFORE_DAYS_MESSAGE', 'You can not cancel/pause the subscription before {{preventDays}} days from your next order date.', 'You can not cancel/pause the subscription before {{preventDays}} days from your next order date.', 3),

-- CYCLE MANAGEMENT
  (uuid(), 'abc', 'en', 'CYCLE_MGMT', 'MIN_CYCLE_TEXT', 'Min Cycle', 'Min Cycle', 1),
  (uuid(), 'abc', 'en', 'CYCLE_MGMT', 'MAX_CYCLE_TEXT', 'Max Cycle', 'Max Cycle', 2),
  (uuid(), 'abc', 'en', 'CYCLE_MGMT', 'EDIT_DELIVERY_INTERVAL_TEXT', 'Edit Delivery Interval', 'Edit Delivery Interval', 3),
  (uuid(), 'abc', 'en', 'CYCLE_MGMT', 'DELIVERY_FREQUENCY_TEXT', 'Delivery Frequency', 'Delivery Frequency', 4),
  (uuid(), 'abc', 'en', 'CYCLE_MGMT', 'CURRENT_BILLING_CYCLE_TEXT', 'Current billing cycle:', 'Current billing cycle:', 5),

-- BANNERS
  (uuid(), 'abc', 'en', 'BANNERS', 'SECURITY_CHALLENGE_BANNER_TEXT', 'Security Challenge Banner', 'Security Challenge Banner', 1);
