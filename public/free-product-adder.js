(function () {
  ...
  const APP = window.freeProductAdderAppUrl || "https://shopify-discount-app-production.up.railway.app";
  let lock = false;
  let lastCart = null;
  ...
  function applyQtyRestrictions(cart) {
    ...
    cart.items.forEach((item, idx) => {
      ...
      if (isFreeGift) {
        ...
        freeGiftVariantIds.push(String(item.variant_id));
      }
    });
    ...
    freeGiftVariantIds.forEach(vid => {
      ...
      elements.forEach(el => {
        const itemContainer = el.closest('.cart-item, [data-cart-item], tr, .cart-row, .cart-line-item');
        if (itemContainer) {
    ...
    while (node = walk.nextNode()) {
      if (node.nodeValue.includes('free_gift_code') || node.nodeValue.includes('_free_gift_code')) {
        const itemContainer = node.parentElement.closest('.cart-item, [data-cart-item], tr, .cart-row, .cart-line-item, cart-drawer-items > div');
        if (itemContainer) {
          const inputs = itemContainer.querySelectorAll('input[type="number"], input.quantity__input, .quantity input');
          inputs.forEach(input => {
            disableQtyInputAndButtons(input);
  ...
  async function refreshCart(cart) {
    ...
    if (sections.length > 0 && (!parsedState || !parsedState.sections)) {
      try {
        ...
        const res = await fetch(`/cart?sections=${encodeURIComponent(sectionQuery)}`);
        if (res.ok) {
  ...
  async function check() {
    ...
    try {
      ...
      if (codes.length > 0) {
        ...
        if (previousCode) {
          console.log("[FreeProductAdder] Discount removed. Previous code was:", previousCode);
          window.localStorage.removeItem('active_discount_code');
          window.localStorage.removeItem(`free_products_added_${previousCode}`);
          window.localStorage.removeItem(`user_removed_free_gift_${previousCode}`);

          // Clear the cookie to prevent checkout auto-applying
          document.cookie = "discount=; Max-Age=0; path=/;";
          document.cookie = "discount=; expires=Thu, 01 Jan 1970 00:00:00 UTC; path=/;";

          // Find if there are any free gifts with property '_free_gift_code' === previousCode
          const updates = {};
          let needsUpdate = false;
          cart.items.forEach(i => {
            if (i.properties && i.properties.free_gift_code === previousCode) {
              updates[i.key] = 0;
              needsUpdate = true;
            }
          });
      ...
      if (changed && toAdd.length) {
        ...
        if (addRes.ok) {
          ...
          console.error("[FreeProductAdder] Failed to add items via /cart/add.js:", addRes.status);
          hideLoader();
        }
      } else {
  ...
  window.addEventListener('click', function(e) {
    ...
    const itemContainer = removeBtn.closest('.cart-item, [data-cart-item], tr, .cart-row, .cart-line-item');
